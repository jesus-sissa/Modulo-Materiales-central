Public Class frm_Compras
    ...
    Private Sub ObtenerIVA()
        ...
        lbl_PorcentajeRetencion.Text = Dt.Rows(0)("IVA") & "%"
        Dt.Dispose()
    End Sub
    ...
    Private Sub btn_Agregar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Agregar.Click
        ...
        If cmb_Proveedor.SelectedValue = 0 Then
            ...
            Exit Sub
        End If
        If Not rdb_Material.Checked And Not rdb_Abierto.Checked And Not rdb_Servicio.Checked Then
        ...
        ElseIf rdb_Servicio.Checked Then
            Lsv_Ventas.Items.Add("")
            Lsv_Ventas.Items(Lsv_Ventas.Items.Count - 1).Tag = 0
            Lsv_Ventas.Items(Lsv_Ventas.Items.Count - 1).SubItems.Add(tbx_Descripcion.Text.Trim)
        End If
    ...
    Private Sub CalcularImportes()
        ...
        'Dim Retencion As Decimal = "0." + tbx_Retencion.Text
        ...
        If Subtotal >= 0 Then
            ...
            'tbx_Total.Text = Format(CDec(tbx_Total.Text) - CDec(Txt_Retencion.Text), "n2")
            btn_Guardar.Enabled = True
        Else
            tbx_SubTotal.Text = Format(Subtotal, "n2")
    ...
    Private Sub btn_Guardar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Guardar.Click
        ...
        Dim Rentencion = Tbx_Retencion.Text
        ...
        Id_MatCompra = Cn_Materiales.fn_ComprasMateriales_Guardar(Lsv_Ventas, Tipo_Orden, cmb_Proveedor.SelectedValue, tbx_Atencion.Text, CDec(tbx_SubTotal.Text), CDec(tbx_IVA.Text), CDec(tbx_Total.Text), rtb_Observaciones.Text.Trim, cmb_Moneda.SelectedValue, Domicilio_Entrega, Condiciones_Pago, CDec(Rentencion), CDec(Txt_Retencion.Text))
    ...
    Private Sub btn_Borrar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Borrar.Click
        ...
        'tbx_Retencion.Enabled = Lsv_Ventas.Items.Count >= 0

        'tbx_Retencion.Text = 0.0
        Call CalcularImportes()
    ...
    Private Sub rdb_Servicio_CheckedChanged(sender As Object, e As EventArgs) Handles rdb_Servicio.CheckedChanged
        SegundosDesconexion = 0

        If rdb_Servicio.Checked Then
            lbl_Material.Visible = False
            tbx_ClaveMaterial.Visible = False
            cmb_Material.Visible = False
            lbl_Descripcion.Visible = True
    ...
    Private Sub cmb_Unidad_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_Unidad.SelectedIndexChanged
        'If cmb_Unidad.SelectedValue = "MILLAR" Then
        '    lb_leyenda.Text = "Colocar la cantida en Piezas"
        'Else
        '    lb_leyenda.Text = "."
        'End If
    End Sub
    ...
    Private Sub Tbx_Retencion_TextChanged_1(sender As Object, e As EventArgs) Handles Tbx_Retencion.TextChanged
        Try

            Dim SubTotal As Decimal = CDec(tbx_SubTotal.Text)
            Dim Iva As Decimal = CDec(tbx_IVA.Text)
            Dim Total As Decimal = CDec(tbx_Total.Text)
            Dim regla As Integer = 100
            If Tbx_Retencion.Text >= 0 And Tbx_Retencion.Text < 100 Then



                'Dim retencion As Decimal = Format((regla - Tbx_Retencion.Text) / regla * (SubTotal + Iva), "n2")

                Dim retencion As Decimal = Format((regla - Tbx_Retencion.Text) / regla * (SubTotal), "n2")

                tbx_Total.Text = Format(retencion + Iva, "n2")

                Txt_Retencion.Text = Format((SubTotal + Iva) - tbx_Total.Text, "n2")

                'tbx_Total.Text = Format((regla - Tbx_Retencion.Text) / regla * (SubTotal + Iva), "n2")

                If Tbx_Retencion.Text = 0 Then
                    tbx_Total.Text = Format(SubTotal + Iva, "n2")

                End If

            Else

                MsgBox("El rango de porcentaje es del 1 al 99.", MsgBoxStyle.Critical, frm_MENU.Text)
                    Lsv_Ventas.Clear()
                    Call CalcularImportes()
                    Tbx_Retencion.Text = ""
                    Txt_Retencion.Text = ""
            ...
            'If Tbx_Retencion.Text >= 0 And Tbx_Retencion.Text < 10 Then

            '    Dim retencion As Decimal = "0.0" + Tbx_Retencion.Text

            '    Txt_Retencion.Text = Format((SubTotal * retencion), "n2")


            '    tbx_Total.Text = Format((SubTotal + Iva) - Txt_Retencion.Text, "n2")
            ...
            'Else
            '    If Tbx_Retencion.Text > -1 And Tbx_Retencion.Text < 100 Then
            '        Dim retencion As Decimal = Format("0." + Tbx_Retencion.Text, "n2")

            '        Txt_Retencion.Text = Format((SubTotal * retencion), "n2")

            '        tbx_Total.Text = Format((SubTotal * retencion) + Iva, "n2")
            '    Else
            '        MsgBox("El rango de porcentaje es del 1 al 99.", MsgBoxStyle.Critical, frm_MENU.Text)
            '        Lsv_Ventas.Clear()
            '        Call CalcularImportes()
